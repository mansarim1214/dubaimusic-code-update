...
router.post('/', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'galleryImages', maxCount: 10 }]), async (req, res) => {
  const { title, description, videoUrl, category } = req.body; // Include category
  ...
  try {
    const newWeddingVIP = new WeddingVIP({ title, description, videoUrl, category, imageUrl, galleryImages }); // Include category in new document
...
router.put('/:id', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'galleryImages', maxCount: 10 }]), async (req, res) => {
  const { title, description, videoUrl, category } = req.body; // Include category in the update
  const updateData = { title, description, videoUrl, category }; // Include category in update data
